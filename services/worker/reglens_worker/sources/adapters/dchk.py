"""Dental Council of Hong Kong source adapter for synthetic RC3 fixtures."""
...
DCHK_JULY_2018_CAVEAT = (
    "From July 14, 2018, the source generally publishes written judgments only where all "
    "or some charges were found guilty. This does not mean unpublished cases did not exist."
)
...
class DchkAdapter(BaseSourceAdapter):
    """Parse DCHK judgment table HTML into normalized source items."""

    source_id = "dchk_judgments"
    adapter_id = "dchk_html_table"
    ...
    def discover(
        ...
        *,
        ...
        seen: set[str] = set()
        for row in _judgment_rows(soup):
            item = self.normalize_item(_raw_row(row, base_url))
...
def _raw_row(row: Tag, base_url: str) -> dict[str, Any]:
    ...
    return {
        ...
        "source_url": urljoin(base_url, detail_link) if detail_link else base_url,
        ...
        "metadata": {"source_page_url": base_url},
    }
...
def _first_pdf_link(row: Tag) -> str | None:
    anchor = row.select_one("a.pdf-link[href]")
    if isinstance(anchor, Tag):
        return str(anchor["href"])
...
def _key(value: str) -> str:
    ...
    if not cleaned:
        raise ValueError("DCHK source row is missing a case reference")
